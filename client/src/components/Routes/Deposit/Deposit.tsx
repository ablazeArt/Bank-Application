...
const Deposit: React.FC = () => {
  ...
  const submitForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        Swal.fire("Congratulations", "Deposit completed", "success");
      })
      .catch((err) => {
        Swal.fire({
          icon: "error",
  ...
  //กำหนดค่าให้state
  useEffect(() => {
    setState({ ...state, action: "Deposit", fromAccount: account.accountNumber, toAccount: account.accountNumber, remain: account.balance + Number(state.amount) });
  ...
  return (
    <section id="withdrawn" className="pb-8">
      <div className="flex text-white py-8">
        ...
        <h1 className="text-2xl py-1 mx-auto">Deposit</h1>
      </div>
      <div>
