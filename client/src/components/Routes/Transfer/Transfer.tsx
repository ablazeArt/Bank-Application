...
const Transfer = () => {
  ...
  const [accountReceive, setAccountReceive] = useState<Account>({
    accountHolder: "",
    accountNumber: "",
    balance: 0,
  });
  ...
  const [receiver, setReceiver] = useState<TransferState>({
    toAccount: "",
    amount: 0,
    action: "",
    fromAccount: "",
  ...
  const { action, fromAccount, toAccount, amount, remain } = transfer;
  ...
  const submitForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        Swal.fire("Congratulations", "Transfer completed", "success");
      })
      .catch((err) => {
        Swal.fire({
          icon: "error",
    ...
    axios
      .post(`http://localhost:5000/transaction`, { action: "Receive from others", fromAccount, toAccount, amount, remain: accountReceive.balance + Number(transfer.amount) })
      .then((response) => {
        setReceiver({ ...receiver, amount: 0 });
      })
      .catch((err) => {
        Swal.fire({
          icon: "error",
    ...
    axios
      .put(`http://localhost:5000/account/${transfer.toAccount}`, { balance: accountReceive.balance + Number(transfer.amount) })
      .then((response) => {})
      .catch((err) => {
        Swal.fire({
          icon: "error",
  ...
  //กำหนดค่าให้state
  useEffect(() => {
    setTransfer({ ...transfer, action: "Transfer to others", fromAccount: account.accountNumber, remain: account.balance - Number(transfer.amount) });
    //get transfer account data
    axios.get(`http://localhost:5000/account/${transfer.toAccount}`).then((response) => {
      setAccountReceive(response.data);
    });
  ...
  return (
    <section id="tranfer" className="pb-8">
      <div className="flex text-white py-8">
        <Link to={`/account/${accountNumberParam}`}>
          <AiOutlineArrowLeft className="text-4xl" />
        </Link>
        <h1 className="text-2xl py-1 mx-auto">Transfer</h1>
      </div>
      <div>
        <h3 className="text-2xl py-1 text-white">From</h3>
      ...
      <form onSubmit={submitForm}>
        <div>
          <h3 className="text-2xl py-1 text-white">To</h3>
          <div className="flex flex-col gap-10 pt-5 pb-10 lg:flex-row lg:flex-wrap">
            <div className="bg-neutral-50 rounded-lg flex-1 text-left md:mb-18">
              <div className="flex py-3 bg-blue-50 rounded-lg border-b border-gray-300">
                <img src="https://image.makewebeasy.net/makeweb/m_200x200/m14BHwk4P/Home/clicknext_logo2x.png?v=202012190947" className="mt-2 ml-6 mr-6 h-6 sm:h-11 sm:mt-0" alt="" />
                <div className="md:text-xl text-base">
                  <h1 className="mt-2 font-semibold">ClickNext Bank</h1>
                </div>
              </div>
              <div className="text-center">
                <input type="text" value={toAccount} onChange={inputValue("toAccount")} placeholder="Account no." className="w-5/6 text-xl font-bold mt-8 border-0 border-b border-blue-500 bg-neutral-50" />
        ...
        <div className="bg-blue-50 pb-10">
          ...
          <div className="text-center">
            ...
            <p className="text-left ml-28 mt-2 text-gray-500 pb-5">Enter account number (10-19 difits)</p>
            <button onClick={updateBalance} type="submit" className="w-5/6 bg-blue-800 text-white p-3 hover:bg-blue-700 rounded-lg">
              Confirm
            </button>
          </div>
        </div>
